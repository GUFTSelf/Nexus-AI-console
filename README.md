...
- a TypeScript/Firebase-authenticated Cloud Run gateway scaffold;
- Android, iOS, and backend continuous-integration workflows.
...
NEXUS_GATEWAY_BASE_URL=https://your-verified-cloud-run-service/
```
...
The `backend/` directory contains a deployable Cloud Run scaffold. It is not a live production service until an owner deploys it and configures Firebase, App Check, allowed origins, and managed credentials. Its deterministic replay route uses the same `nexus.execution.v1` canonical JSON and fixed test vector as Android and iOS.
...
Production requests require both a Firebase Auth bearer token and a Firebase App Check token. The mobile client keeps the gateway disabled when `NEXUS_GATEWAY_BASE_URL` is blank, and release builds reject a non-HTTPS gateway URL.
