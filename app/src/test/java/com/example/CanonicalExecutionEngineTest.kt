...
class CanonicalExecutionEngineTest {
    ...
    fun `executes ordered arithmetic and returns eighteen`() {
        ...
        val independentlyDerivedHash = MessageDigest.getInstance("SHA-256")
            .digest(result.canonicalRecord.canonicalJson.toByteArray(StandardCharsets.UTF_8))
            .joinToString(separator = "") { byte -> "%02x".format(byte) }
        assertEquals(independentlyDerivedHash, result.canonicalRecord.sha256)
    }
    ...
    @Test
    fun `integer-only execution rejects a non-integral operand`() {
        val failure = runCatching {
            CanonicalExecutionEngine.execute(
                "Using integer arithmetic, start with 7 and add 0.5."
            )
        }
