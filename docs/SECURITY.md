...
The current direct Gemini REST client is restricted to Android debug builds. The `backend/` gateway scaffold implements the initial server boundary, but it is not production-enabled until owner-controlled Firebase and Cloud Run configuration is completed. A production gateway must:
